			    ...
			    unsigned int protoff, int *hotdrop)
{
	...
	kttamp.tv.sec = skb->tstamp.tv.sec;
	kttamp.tv.nsec = skb->tstamp.tv.nsec;

	stamp = ktime_to_ns(kttamp);
...
#else
	{
		if (info->flags & XT_TIME_TZ_UTCP) {	// UTC+X
			stamp += 3600 * ((info->flags >> 1) & 0x0F);	// convert utc to local TZ
			if (info->flags & XT_TIME_TZ_MINUTE)
				stamp += 1800;	//30 minutes
		}
		else {
			stamp -= 3600 * ((info->flags >> 1) & 0x0F);
			if (info->flags & XT_TIME_TZ_MINUTE)
				stamp -= 1800;	//30 minutes
		}
	}
