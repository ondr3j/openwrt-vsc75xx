...
extern struct net_device *brlan_dev;
static inline struct net_device *check_ppp_change_prefix_if(struct net_device *dev)
{
	if ((dev->flags & (IFF_UP|IFF_POINTOPOINT)) == (IFF_UP|IFF_POINTOPOINT) &&
		brlan_dev != NULL) {
		return brlan_dev;
	}
	return dev;
...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len)
{
	...
	dev = check_ppp_change_prefix_if(dev);
