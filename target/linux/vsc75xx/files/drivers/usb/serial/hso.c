...
/* fanny : add interface number parameter */
...
static struct hso_device *hso_create_net_device(struct usb_interface *interface, u32 port_spec)
{
...
#else
	{
		...
		if (dual3G == 1) {
			if (hso_dev->usb->portnum == 3) 	// 2130 bottom usb port num = 3
				strcpy(dev_name, "ppp1");
			else
				strcpy(dev_name, "ppp0");
		} else	
			sprintf(dev_name, "ppp%d", inf_num);
		printk("!!!!!!!!!!!!hso driver : interface name change to : %s!!!!!!!\r\n", dev_name);
		net = alloc_netdev(sizeof(struct hso_net), dev_name, hso_net_init);
		if (!net) {
			dev_err(&interface->dev, "Unable to create ethernet device\n");
			goto exit;
		}
...
// fanny : setup hso ppp interface number. 3G wan : ppp0, 3G backup : ppp1.
extern void (*set_hso_driver_inf_num)(int param); 
void set_hso_driver_inf_num_fun(int param)
{
	if (param==2) {
		printk("Hso driver : inf_num set as ppp%d!!!!!!!!!!\r\n", param);
		dual3G=1;
	} else if (param >=0 && param <=1) { // param = 0 or 1
		printk("Hso driver : inf_num set as ppp%d!!!!!!!!!!\r\n", param);
		inf_num = param;
		dual3G=0;
	}	
...
static int __init hso_init(void)
{
	...
	set_hso_driver_inf_num = set_hso_driver_inf_num_fun; //fanny
...
static void __exit hso_exit(void)
{
	...
	set_hso_driver_inf_num = NULL; //fanny
	tty_unregister_driver(tty_drv);
...
MODULE_PARM_DESC(inf_num, "Set the device driver interface number");
module_param(inf_num, int, S_IRUGO | S_IWUSR);
